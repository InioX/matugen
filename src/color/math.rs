...
use material_colors::color::{Lab, Rgb};
use std::str::FromStr;
...
pub fn get_color_distance_lab_from_str(c1: &str, c2: &str) -> Result<f64, Report> {
    let c1 = Lab::from(Rgb::from_str(c1)?);
    let c2 = Lab::from(Rgb::from_str(c2)?);

    Ok(get_color_distance_lab(&c1, &c2))
}

pub fn luminance(c: &ColorSysRgb) -> f32 {
    0.2126 * c.red() as f32 + 0.7152 * c.green() as f32 + 0.0722 * c.blue() as f32
}

pub fn lightness(c: &ColorSysRgb) -> f32 {
